window.onload = function () {
	...
	if (filters.length) { // có filter
		...
		addKhungSanPham('CÁC SẢN PHẨM BÁN CHẠY', yellow_red , ['star=3', 'sort=rateCount-decrease'], soLuong, div);
		addKhungSanPham('Nhẫn', yellow_red, ['promo=moiramat', 'sort=rateCount-decrease'], soLuong, div);
		addKhungSanPham('Bông tai', green, ['promo=giareonline', 'sort=rateCount-decrease'], soLuong, div);
		addKhungSanPham('Dây chuyền', blue, ['promo=giamgia'], soLuong, div);
		addKhungSanPham('GIÁ RẺ CHO MỌI NHÀ', green, ['price=0-1500000', 'sort=price'], soLuong, div);
	}

	// Thêm chọn mức giá
	addPricesRange(0, 10000000);
	addPricesRange(10000000, 50000000);
	addPricesRange(50000000, 0);
	...
	for (var i = 0; i < filters.length; i++) {
		...
		switch (dauBang[0]) {
			...
			case 'phan_loai':
				...
				if (saveFilter) filtersFromUrl.phan_loai = dauBang[1];
				break;
...
function timKiemTheoCongTySanXuat(list, tenCongTy, soluong) {
	...
	for (var i = 0; i < list.length; i++) {
		if (list[i].phan_loai.toUpperCase().indexOf(tenCongTy.toUpperCase()) >= 0) {
			result.push(list[i]);
			count--;
			if (count <= 0) break;
		}
...
function addAllChoosedFilter() {
	...
	if (filtersFromUrl.phan_loai != '')
		addChoosedFilter('phan_loai', filtersFromUrl.phan_loai);
...
// Thêm hãng sản xuất
function addphan_loai(img, namephan_loai) {
	var link = createLinkFilter('add', 'phan_loai', namephan_loai);
	var new_tag = `<a href=` + link + `><img src=` + img + `></a>`;

	document.getElementsByClassName('phan_loaiMenu')[0].innerHTML +=new_tag;
