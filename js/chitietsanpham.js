...
function phanTich_URL_chiTietSanPham() {
    ...
    // Đổi title
    document.title = nameProduct + ' - Trang sức PNJ';
    ...
    var s = addThongSo('size', sanPhamHienTai.detail.size);
    s += addThongSo('màu sắc', sanPhamHienTai.detail.color);
    s += addThongSo('số lượng còn lại', sanPhamHienTai.detail.quantity);
...
function suggestion(){
    ...
    .map(sanPham => {
        ...
        let giaTienGanGiong = Math.abs(giaSanPham - giaSanPhamHienTai) < 200000;
        ...
        let cungHangSanXuat = sanPham.phan_loai ===  sanPhamHienTai.phan_loai
